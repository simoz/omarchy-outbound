...
cc -std=c11 -O2 -Wall -Wextra -Werror -c "$source_dir/native/crypt.c" -o crypt.o
cp "$library_dir/libmaxminddb.a" maxmind.o
# Spinel always links -lcrypt; crypt.o resolves the runtime's reference so that
# --as-needed can drop the library (see native/crypt.c).
"$compiler" --cc="cc -Wl,--as-needed" "$source_dir/main.rb" -o "$binary.new" >&2
