...
].each { |pair| check(Scope.classify(Native.outbound_ip_hex(pair[0])) == pair[1], "address scope") }

def coverage
  {"ipv4" => nil, "ipv6" => nil, "namespace" => "current", "omittedRows" => 0, "truncated" => false,
   "processes" => {"denied" => 0, "races" => 0, "errors" => 0, "ownersOmitted" => 0, "timedOut" => false, "scanLimited" => false}}
end

row = {"family" => 2, "local" => {"address" => "127.0.0.1", "port" => 10},
       "remote" => {"address" => "192.0.2.1", "port" => 20}, "state" => 1, "uid" => 1000,
       "inode" => 7, "cookie" => "ffffffffffffffff"}
geo = Geo.new("")
engine = Engine.new(geo)
owners = {7 => [{"pid" => 1, "startTimeTicks" => "8", "name" => "Browser"},
                {"pid" => 2, "startTimeTicks" => "9", "name" => "Browser"},
                {"pid" => 3, "startTimeTicks" => "10", "name" => "Helper"}]}
a = engine.snapshot("a", [row], owners, coverage)
b = engine.snapshot("b", [row], owners, coverage)
check(a["connections"][0]["id"] == b["connections"][0]["id"], "stable fallback")
check(a["aggregates"]["applications"] == {"Browser" => 1, "Helper" => 1}, "shared ownership aggregation")
engine.snapshot("empty", [], {}, coverage)
c = engine.snapshot("c", [row], {}, coverage)
check(a["connections"][0]["id"] != c["connections"][0]["id"], "retired fallback")
failed = coverage
failed["ipv4"] = "interrupted"
engine.snapshot("failed", [], {}, failed)
d = engine.snapshot("d", [row], {}, coverage)
check(c["connections"][0]["id"] != d["connections"][0]["id"], "failed observation retires fallback")
truncated = coverage
truncated["truncated"] = true
truncated["omittedRows"] = 1
engine.snapshot("truncated", [row], {}, truncated)
...
if ARGV.length > 0
  ...
  check(result["owners"][7].all? { |o| o["startTimeTicks"] == "123" && o["name"] == "船🦀name" }, "proc names and identity")
end
if ARGV.length > 1
  geo = Geo.new(ARGV[1])
  ...
  ["8.8.8.8", "::ffff:8.8.8.8", "2001:4860::8888"].each do |ip|
    hex = Native.outbound_ip_hex(ip)
    check(geo.lookup(ip, hex, Scope.classify(hex)) == "IT", "synthetic public lookup")
  end
  ["127.0.0.1", "192.0.2.1", "fc00::1"].each do |ip|
    hex = Native.outbound_ip_hex(ip)
    check(geo.lookup(ip, hex, Scope.classify(hex)).nil?, "special-use exclusion")
  end
